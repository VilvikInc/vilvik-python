#!/bin/bash
# Publish the `vilvik` Python SDK to PyPI (or TestPyPI).
#
# This is the manual fallback; tagged releases publish automatically via
# GitHub Actions. Run it from anywhere, it resolves paths relative to its
# own location and uses whatever Python environment is currently active
# (installing `build` + `twine` into it if they are missing).
#
# Pipeline:
...
#   6. Pause so you can `pip install -i https://test.pypi.org/simple/ vilvik`
...
if confirm "Upload to TestPyPI now?"; then
  ...
  info "      --extra-index-url https://pypi.org/simple/ vilvik"
  echo
...
warn "re-upload the same filename — you'd have to bump the version in"
warn "pyproject.toml and rebuild. Make sure the TestPyPI smoke test passed."
if confirm "Upload to production PyPI now?"; then
  python -m twine upload dist/*
  success "Uploaded to PyPI: https://pypi.org/project/vilvik/"
else
